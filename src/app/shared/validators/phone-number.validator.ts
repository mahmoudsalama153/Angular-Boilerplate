 ...
 */
const COUNTRY_PHONE_PATTERNS: Record<
  string,
  { pattern: RegExp; minLength: number; maxLength: number; description: string }
> = {
  // Saudi Arabia
  '+966': {
    pattern: /^[0-9]{9}$/,
  ...
  '+965': {
    ...
    minLength: 8,
    maxLength: 8,
    description: '8 digits (e.g., 50123456)',
  },
  // Qatar
  '+974': {
    pattern: /^[0-9]{8}$/,
  ...
  '+1': {
    ...
    minLength: 10,
    maxLength: 10,
    description: '10 digits (e.g., 2015551234)',
  },
  // United Kingdom
 ...
 */
function getCountryValidationPattern(
  countryCode: string | null | undefined,
): { pattern: RegExp; minLength: number; maxLength: number; description: string } | null {
  if (!countryCode) {
    return null;
  }
 ...
 */
export function phoneNumberPatternValidator(): (
